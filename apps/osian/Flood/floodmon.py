# Monitor Flood messages when BridgePppRadio is running.
#
import socket
import select
import binascii
...
rv = socket.getaddrinfo('ff12::1', port, *sock_args)
mc_sockaddr = rv[0][4]
...
payload_layout = '<IIIH'
payload_expects = struct.calcsize(payload_layout)
...
while True:
    ...
    if rc:
        ...
        if len(payload) >= struct.calcsize(payload_layout):
            (localTime_bms, transmitAttempts, transmitSuccesses, retryCount) = struct.unpack(payload_layout, payload[:payload_expects])
            print '%s: len %u up %u tx %u of %u retry %u' % (sender[0], len(payload), localTime_bms, transmitSuccesses, transmitAttempts, retryCount)
